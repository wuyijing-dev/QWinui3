    ...
    run_sh.write_text(
        "\n".join(
            [
                ...
                '# Prefer Wayland CSD when unset — Gallery calls configurePlatformEnvironment()',
                '# which picks wayland;xcb. Do not force xcb (that stuck packages on XWayland).',
    ...
    readme.write_text(
        "\n".join(
            [
                ...
                "```",
                "",
                "Leave `QT_QPA_PLATFORM` unset so the Gallery picks **Wayland first**",
                "(falls back to X11). Force a backend only when debugging:",
                "",
                "```bash",
                "QT_QPA_PLATFORM=wayland ./run-gallery.sh",
                "QT_QPA_PLATFORM=xcb ./run-gallery.sh",
                "```",
                "",
                "Fluent client-side chrome needs compositor SSD off (default):",
                "`QT_WAYLAND_DISABLE_WINDOWDECORATION=1`.",
                "",
                "Nav / settings style apps: use `BackdropSolid` (or let",
