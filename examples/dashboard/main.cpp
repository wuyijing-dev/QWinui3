...
int main(int argc, char *argv[])
{
    ...
    QCoreApplication::setApplicationName(QStringLiteral("DashboardExample"));
    QGuiApplication::setDesktopFileName(QStringLiteral("org.qwinui3.example.dashboard"));
    ...
    QObject::connect(
        ...
        Qt::QueuedConnection);
    engine.loadFromModule("QWinUI3.Examples.Dashboard", "Main");
    if (engine.rootObjects().isEmpty()) {
        qWarning() << "Failed to load QWinUI3.Examples.Dashboard/Main";
        return -1;
    }
    return app.exec();
}
